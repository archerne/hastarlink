...
class DishGrpcText:
    ...
    def parse_args(self):
        parser = self._dish_common.create_arg_parser(
    ...
    def open_out_file(self, opts, mode):
        if opts.out_file == "-":
    ...
    def print_header(self, opts, print_file):
        header = ["datetimestamp_utc"]

        def header_add(names):
            for name in names:
                name, start, end = self._dish_common.BRACKETS_RE.match(
                    name).group(1, 4, 5)
                if start:
    ...
    def get_prior_counter(self, opts, gstate):
        ...
        # great.
        try:
            with self.open_out_file(opts, "r") as csv_file:
                header = csv_file.readline().split(",")
                column = header.index(self.COUNTER_FIELD)
                last_line = None
    ...
    def loop_body(self, opts, gstate, print_file, shutdown=False):
        csv_data = []
        ...
        def cb_data_add_item(name, val, category):
            if opts.verbose:
                csv_data.append("{0:22} {1}".format(
                    self.VERBOSE_FIELD_MAP.get(name, name) + ":", xform(val)))
            else:
        ...
        def cb_data_add_sequence(name, val, category, start):
            if opts.verbose:
                csv_data.append("{0:22} {1}".format(
                    self.VERBOSE_FIELD_MAP.get(name, name) + ":",
    ...
    def main(self, opts):
        ...
        if opts.print_header:
            try:
                with self.open_out_file(opts, "a") as print_file:
                    lines.append(self.print_header(opts, print_file))
            except OSError as e:
                logging.error("Failed opening output file: %s", str(e))
                rc = 1
            # sys.exit(rc)

        gstate = self._dish_common.GlobalState(target=opts.target)
        if opts.out_file != "-" and not opts.skip_query and opts.history_stats_mode:
            self.get_prior_counter(opts, gstate)

        try:
            print_file = self.open_out_file(opts, "a")
        except OSError as e:
            logging.error("Failed opening output file: %s", str(e))
            # sys.exit(1)
        ...
        try:
            ...
            while True:
                ...
                if opts.loop_interval > 0.0:
                    ...
                    break
        except (KeyboardInterrupt, self.Terminated):
            pass
        finally:
            self.loop_body(opts, gstate, print_file, shutdown=True)
            print_file.close()
            gstate.shutdown()
        return lines
        # sys.exit(rc)

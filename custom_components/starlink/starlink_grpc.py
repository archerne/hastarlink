...
class StarlinkGrpc:
    ...
    def resolve_imports(self, channel: grpc.Channel):
    ...
    def call_with_channel(self, function, *args, context: Optional[ChannelContext] = None, **kwargs):
    ...
    def status_field_names(self, context: Optional[ChannelContext] = None):
        ...
        if imports_pending:
            try:
                self.call_with_channel(self.resolve_imports, context=context)
            except grpc.RpcError as e:
                raise self.GrpcError(e)
        alert_names = []
    ...
    def status_field_types(self, context: Optional[ChannelContext] = None):
    ...
    def get_status(self, context: Optional[ChannelContext] = None):
        ...
        def grpc_call(channel):
            if imports_pending:
                self.resolve_imports(channel)
            stub = device_pb2_grpc.DeviceStub(channel)
            response = stub.Handle(device_pb2.Request(
                get_status={}), timeout=self.REQUEST_TIMEOUT)
            return response.dish_get_status

        return self.call_with_channel(grpc_call, context=context)

    def get_id(self, context: Optional[ChannelContext] = None):
    ...
    def status_data(self, context: Optional[ChannelContext] = None):
    ...
    def history_bulk_field_names(self):
    ...
    def history_bulk_field_types(self):
    ...
    def history_ping_field_names(self):
        """Deprecated. Use history_stats_field_names instead."""
        return self.history_stats_field_names()[0:3]

    def history_stats_field_names(self):
    ...
    def history_stats_field_types(self):
    ...
    def get_history(self, context: Optional[ChannelContext] = None):
        ...
        def grpc_call(channel: grpc.Channel):
            ...
            stub = device_pb2_grpc.DeviceStub(channel)
            response = stub.Handle(device_pb2.Request(
                get_history={}), timeout=self.REQUEST_TIMEOUT)
            return response.dish_get_history

        return self.call_with_channel(grpc_call, context=context)

    def _compute_sample_range(self, history, parse_samples: int, start: Optional[int] = None, verbose: bool = False):
        ...
        if start_offset < end_offset:
            ...
            sample_range = chain(range(start_offset, samples),
                                 range(0, end_offset))

        return sample_range, current - start, current

    def concatenate_history(self, history1, history2, samples1: int = -1, start1: Optional[int] = None, verbose: bool = False):
        ...
        unwrapped = self.UnwrappedHistory()
        for field in self.HISTORY_FIELDS:
            setattr(unwrapped, field, [])
        unwrapped.unwrapped = True

        sample_range, ignore1, ignore2 = self._compute_sample_range(  # pylint: disable=unused-variable
            history1, samples1, start=start1)
        for i in sample_range:
            for field in self.HISTORY_FIELDS:
                getattr(unwrapped, field).append(getattr(history1, field)[i])

        sample_range, ignore1, ignore2 = self._compute_sample_range(
            history2, new_samples)  # pylint: disable=unused-variable
        for i in sample_range:
            for field in self.HISTORY_FIELDS:
    ...
    def history_bulk_data(self, parse_samples: int, start: Optional[int] = None, verbose: bool = False, context: Optional[ChannelContext] = None, history=None):
        ...
        sample_range, parsed_samples, current = self._compute_sample_range(history,
                                                                           parse_samples,
                                                                           start=start,
                                                                           verbose=verbose)
    ...
    def history_ping_stats(self, parse_samples: int, verbose: bool = False, context: Optional[ChannelContext] = None):
        """Deprecated. Use history_stats instead."""
        return self.history_stats(parse_samples, verbose=verbose, context=context)[0:3]

    def history_stats(self, parse_samples: int, start: Optional[int] = None, verbose: bool = False, context: Optional[ChannelContext] = None, history=None):
        ...
        for i in sample_range:
            ...
            if d >= 1:
                ...
                if init_run_length is None:
                    ...
                    if run_length <= 60:
                        ...
                        minute_runs[min((run_length-1) // 60 - 1, 59)
                                    ] += run_length
                run_length = 0
            ...
            if d == 0.0:
                ...
                if down + up > 500000:
                    rtt_buckets[min(
                        14, int(math.log2((down+up) / 500000)))].append(rtt)
                else:
    ...
    def get_obstruction_map(self, context: Optional[ChannelContext] = None):
    ...
    def obstruction_map(self, context: Optional[ChannelContext] = None):
        ...
        """
        try:
            map_data = self.get_obstruction_map(context)
        except grpc.RpcError as e:
            raise self.GrpcError(e)
    ...
    def reboot(self, context: Optional[ChannelContext] = None) -> None:
        ...
        def grpc_call(channel: grpc.Channel) -> None:
            if imports_pending:
                self.resolve_imports(channel)
            stub = device_pb2_grpc.DeviceStub(channel)
            stub.Handle(device_pb2.Request(reboot={}),
                        timeout=self.REQUEST_TIMEOUT)
            # response is empty message in this case, so just ignore it
            return None

        try:
            self.call_with_channel(grpc_call, context=context)
        except grpc.RpcError as e:
            raise self.GrpcError(e)

    def set_stow_state(self, unstow: bool = False, context: Optional[ChannelContext] = None) -> None:
        ...
        def grpc_call(channel: grpc.Channel) -> None:
            ...
            stub.Handle(device_pb2.Request(
                dish_stow={"unstow": unstow}), timeout=self.REQUEST_TIMEOUT)
            # response is empty message in this case, so just ignore it
            return None
